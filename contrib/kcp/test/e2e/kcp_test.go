...
func TestKCPIntegration(t *testing.T) {
	t.Logf("Testing KCP integration, tempdir: %s", t.TempDir())
	...
	framework.ApplyFiles(t,
		...
		"../../deploy/examples/apiresourceschema-cowboys.yaml",
		"../../deploy/examples/apiresourceschema-sheriffs.yaml",
	)
...
func testKCPResourceSync(t *testing.T, consumerCfg, providerCfg *rest.Config) {
	...
	t.Run("instance created downstream syncs upstream", func(t *testing.T) {
		...
		cowboyInstance := &unstructured.Unstructured{
		...
		kcptestinghelpers.Eventually(t, func() (bool, string) {
			_, err := consumerClient.Namespace("default").Create(t.Context(), cowboyInstance, metav1.CreateOptions{})
		...
		require.Equal(t, 1, len(instances.Items), "Expected exactly one cowboy instance on provider side")
	})
	...
	t.Run("instance spec updated downstream syncs upstream", func(t *testing.T) {
		...
		require.Eventually(t, func() bool {
			...
			return err == nil
		}, wait.ForeverTestTimeout, 5*time.Second, "waiting for cowboy spec to be updated on consumer side")
		...
		}, wait.ForeverTestTimeout, 5*time.Second, "waiting for cowboy spec update to sync to provider side")
	})
	...
	t.Run("instance deleted downstream is deleted upstream", func(t *testing.T) {
		...
		}, wait.ForeverTestTimeout, 5*time.Second, "waiting for cowboy instance to be deleted on provider side")
	})
}
